...
大语言模型智能体是近两年研究和工程领域极其火爆的课题，在本系列的上一集#link("https://zhuanlan.zhihu.com/p/2016613752884196375", "Codex源码解析")当中我们就以Codex为例解析了大语言模型智能体的基本实现方式。但在其中仍然缺少了一些重要的内容，也就是关于智能体的扩展性问题。在本篇当中我们就着重补齐这一部分，着重去讲近期被广泛应用的Agent Skills机制。

简而言之，Agent Skills的构建思路很像是von Neumann机，也即是当前几乎所有计算机的实现思路。它将程序也作为一种数据进行处理，而这在大语言模型当中的具体实现就是：Agent Skill当中会封装好使用文本描述的工作流程以及执行任务所需的各种资源，供智能体参考使用。而在具体实现上，Agent Skills也无非只是一些文本协议以及标准规范。我们这里就做一些简单介绍，并且讲解一下它在mini-codex当中的实现情况。更具体的内容可以参考#link("Agent Skills官方文档", "https://agentskills.io/home")，本文主要也参考自官方文档以及Codex当中的实现。
...
关于Skills构建当中的更多标准细节可以参考#link("https://agentskills.io/specification", "官方文档Specification章节")。
...
在这一章节我们就来介绍一下Skills功能在#link("https://github.com/Carlos-Mero/mini-codex", "mini-codex")当中的具体实现。实际上Skills本身只是一些预制的指令以及相关资源，只是具体实现起来仍然会有非常多的麻烦需要处理。这在#link("https://agentskills.io/client-implementation/adding-skills-support", "Skills官网指导")当中都有详细的介绍，不过我们都不对这些特性做额外处理了。
...
我们后续可以通过一个具体的第三方skill来测试它的工作情况。这里我们选用了#link("https://github.com/JinGuYuan/jinguyuan-dumpling-skill", "金谷园饺子馆Skill")来用作测试。金谷园是我最喜欢的饺子馆，吃过的都说好！而这个skill则可以帮助大模型准确地回答和金谷园饺子馆相关的各种问题，包括在哪吃，几点开门，怎么排队等等。这些信息都是通过MCP协议实时获取的，因此实效性非常棒。
