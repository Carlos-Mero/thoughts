...
大约自2025年以后随着各种大语言模型智能体代码工具的蓬勃发展，Vibe Coding这一概念也随之广泛流行起来。在实践上它已经能够有效加速很多代码工作，我本身也是其重度用户，并且经常使用 [OpenAI Codex](https://github.com/openai/codex) 来辅助日常工作。

Codex就是几个最流行的coding agent之一，同时更棒的是它本身的代码完全开源（准确来说是只有Codex cli版本开源）。当前我也正在学习Codex的源码。但原Codex代码已经高度封装化，和工程化，这对于实际使用来说自然相当重要，但同时却也会使得直接学习其工程内容变得困难。其核心实现原理的简洁与美妙也往往被这些繁杂的工程实现所掩盖。我因此想到或许可以尝试从零开始编写一个最简化的 [mini-codex](https://github.com/Carlos-Mero/mini-codex)，尝试用最少的代码复刻出codex的主要功能。我于是做了这个小玩具，并围绕它做了一些简单的研究工作。
...
当我们通过API调用将工具相关的定义传输给vllm、sglang或者各家的私有语言模型推理引擎后，它们所做的事情也是将 `tools` 重新解析为文本并以特定格式插入到prompt当中（其中可能会附带一些特殊token）。很多时候它其实也就是在system prompt下自动插入了如下这段文本（参见sglang当中DeepSeek-V3.2的文本格式模板 [tool_chat_template_deepseekv32.jinja](https://github.com/sgl-project/sglang/blob/6ed996bf65b2e29d871e7dd306d9670197ecbb82/examples/chat_template/tool_chat_template_deepseekv32.jinja#L4)）
...
此外值得指出的是，当前推理模型是否启用深度思考，以及思考强度参数也都是通过prompt方法去实现的。在完成文本格式化并从语言模型中采样生成后，推理引擎会另外从语言模型的回复当中提取出结构化的工具调用信息。这一过程对于很多模型来说也都有所不同，其中同样以DeepSeek-V3.2为例，其在sglang当中的工具调用的提取脚本就在 [deepseekv32_detector.py](https://github.com/sgl-project/sglang/blob/cc73355a1fbd2607743da32b7082edc3b1040a01/python/sglang/srt/function_call/deepseekv32_detector.py#L4)。
...
在实现mini-codex后我也对它进行了一些实验，尤其关注了其与原版OpenAI Codex的能力对比。我在 [mini-codex-examples](https://github.com/Carlos-Mero/mini-codex-examples) 这个GitHub仓库当中放了原版codex和mini-codex制作的小网页游戏项目各三个。可以看到在这种不太长的常规测试项目当中，所实现的工程项目结果更多依赖于基础模型的能力，而这两个编码智能体框架所能带来的差异并不大。尽管如此，这一简单的agent框架仍是必要的，否则如果大模型并不能自主读取以及编辑项目文件，它自然也就无从自动完成这些开发工作了。

![mini-codex flappybird](../src/media/mini-codex-gpt-5.4-flappybird.jpeg)
![codex flappybird](../src/media/codex-gpt-5.4-flappybird.jpeg)

*mini-codex（左）和 OpenAI Codex（右）在相同prompt下制作的flappybird游戏对比。更多示例和可运行版本的代码请参见 [mini-codex-examples](https://github.com/Carlos-Mero/mini-codex-examples)。*
